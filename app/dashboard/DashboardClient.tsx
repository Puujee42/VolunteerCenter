...
const IconMap: any = {
  ...
  FaMedal, FaClipboardList, FaLightbulb, FaHandsHelping,
  FaChartPie, FaCheckCircle, FaLeaf, FaPaw, FaGraduationCap,
  FaFlag, FaUser, FaCalendarAlt, FaHandshake
};
  ...
  return (
    <div className="min-h-screen bg-slate-50 p-6 md:p-12 pt-50">
      <div className="max-w-7xl mx-auto pt-35">
        ...
        {/* Header */}
        <motion.div className="mb-10 flex flex-col md:flex-row items-center justify-between gap-6" initial={{ opacity: 0, y: -20 }} animate={{ opacity: 1, y: 0 }}>
          <div className="flex items-center gap-4">
            ...
            <div>
              <h1 className="text-3xl md:text-5xl font-bold text-slate-800">
                Welcome, <span className="text-blue-600">{dbUser.fullName || user.firstName}!</span>
              </h1>
          ...
          <div className="flex gap-3">
            ...
            <button
                ...
                disabled={isDeleting}
                className="flex items-center gap-2 px-4 py-2 text-sm font-bold text-red-600 bg-red-50 border border-red-100 rounded-lg hover:bg-red-100 transition-colors disabled:opacity-50"
            >
                {isDeleting ? '...' : <><FaTrash /> Delete Account</>}
            </button>
          </div>
...
const ProfileCard = ({ details }: { details: any }) => (
  <motion.div className="bg-white rounded-2xl p-6 shadow-sm border border-slate-100" variants={fadeInUp}>
    ...
    <div className="space-y-4">
        <ProfileRow label="Age" value={details.age || 'N/A'} icon={FaBirthdayCake} />
        <ProfileRow label="Location" value={`${details.province || 'N/A'}, ${details.district || ''}`} icon={FaMapMarkedAlt} />
        <ProfileRow label="Program" value={details.program || 'N/A'} icon={FaTasks} />
        {details.school && <ProfileRow label="School" value={details.school} icon={FaGraduationCap} />}
        {details.partner && <ProfileRow label="Organization" value={details.partner} icon={FaHandshake} />}
    </div>
  </motion.div>
);
                ...
                history.map((item, idx) => {
                    ...
                    return (
                        <div key={idx} className="relative pl-6 pb-2 group">
                            <div className="absolute -left-[25px] top-0 bg-white border-2 border-blue-500 rounded-full p-1 text-blue-500 text-xs"><ItemIcon /></div>
                            <div className="flex justify-between items-start">
                                <div>
                                    <span className="text-xs text-slate-400 font-semibold uppercase tracking-wider">{item.date}</span>
                                    <h4 className="font-bold text-slate-800">{item.title}</h4>
                                </div>
                                <button onClick={() => onLeave(item.id, itemType)} disabled={loadingLeave === item.id} className="opacity-0 group-hover:opacity-100 transition-opacity text-red-400 hover:text-red-600 hover:bg-red-50 p-1.5 rounded-full" title="Leave this activity">
...
const OpportunityHub = ({ items, onJoin, loadingId }: any) => (
    <motion.div className="bg-white rounded-2xl p-6 shadow-sm border border-slate-100" variants={fadeInUp}>
        <div className="flex items-center justify-between mb-4">
            <h3 className="text-lg font-bold text-slate-800 flex items-center gap-2"><FaHandsHelping className="text-blue-500" /> Discover</h3>
            <span className="text-xs font-bold bg-blue-100 text-blue-600 px-2 py-1 rounded-full">{items.length} New</span>
        </div>
        {items.length === 0 ? (
            <div className="text-center text-slate-400 py-6 text-sm border-2 border-dashed border-slate-100 rounded-xl"><FaCheckCircle className="mx-auto text-2xl text-green-400 mb-2" /> You've joined everything!</div>
        ) : (
            <div className="space-y-3">
                {items.slice(0, 4).map((item: any, i: number) => {
                    ...
                    let spotsLeft = 0;
                    if (isEvent) {
                        isFull = (item.registered || 0) >= item.capacity;
                        spotsLeft = item.capacity - (item.registered || 0);
                    } else {
                    ...
                    return (
                        <div key={i} className="p-3 rounded-xl border border-slate-100 hover:border-blue-300 transition-all bg-slate-50/50 flex flex-col gap-2 relative">
                            <span className={`absolute top-2 right-2 text-[10px] uppercase font-bold px-1.5 py-0.5 rounded ${isEvent ? 'bg-purple-100 text-purple-600' : 'bg-green-100 text-green-600'}`}>{isEvent ? "Event" : "Job"}</span>
                            <div>
                                ...
                                <div className="flex justify-between items-center mt-1 text-xs text-slate-500">
                                    ...
                                    <span className={isFull ? "text-red-500" : "text-green-600 font-medium"}>{spotsLeft} spots left</span>
                                </div>
                            </div>
                            <button onClick={() => onJoin(item)} disabled={isFull || loadingId === item.id} className={`w-full py-1.5 text-xs font-bold rounded flex items-center justify-center gap-1 transition-colors ${ isFull ? "bg-gray-200 text-gray-400 cursor-not-allowed" : "bg-blue-600 text-white hover:bg-blue-700" }`}>
...
const ProfileRow = ({ label, value, icon: Icon }: any) => (
    <div className="flex items-center justify-between p-3 bg-slate-50 rounded-lg">
        <div className="flex items-center gap-3"><div className="p-2 bg-white rounded-md text-slate-400 shadow-sm"><Icon /></div><span className="text-sm text-slate-500 font-medium">{label}</span></div>
    ...
    return (
        <motion.div className="bg-gradient-to-br from-blue-600 to-indigo-700 rounded-2xl p-8 text-white shadow-lg relative overflow-hidden" variants={fadeInUp}>
            <div className="relative z-10 flex items-center gap-6">
                <div className="p-4 bg-white/20 backdrop-blur-sm rounded-full"><RankIcon className="text-5xl text-yellow-300 drop-shadow-md" /></div>
                <div className="flex-1">
                    <p className="text-blue-100 font-medium mb-1">Current Rank</p>
                    <h2 className="text-3xl font-bold mb-2">{rank?.current || 'Bronze'}</h2>
                    <div className="flex items-center gap-2 text-sm text-blue-100 mb-2"><span>Next Level Progress</span><span className="font-bold">{rank?.progress || 0}%</span></div>
                    <div className="w-full bg-black/20 h-2 rounded-full overflow-hidden"><div className="h-full bg-yellow-400 rounded-full" style={{ width: `${rank?.progress || 0}%` }}></div></div>
                </div>
            </div>
...
const StrengthsRadarChart = ({ strengths }: any) => (
    <motion.div className="bg-white rounded-2xl p-6 shadow-sm border border-slate-100" variants={fadeInUp}>
        <h3 className="text-lg font-bold text-slate-800 mb-4 flex items-center gap-2"><FaChartPie className="text-blue-500" /> Skills Radar</h3>
...
const RecommendationsCard = ({ recommendations }: any) => (
    <motion.div className="bg-gradient-to-br from-indigo-900 to-slate-900 rounded-2xl p-6 text-white shadow-lg" variants={fadeInUp}>
        <h3 className="text-lg font-bold mb-4 flex items-center gap-2"><FaLightbulb className="text-yellow-400" /> Recommended</h3>
        <div className="space-y-4">
            {(recommendations || []).map((rec: any, i: number) => {
                ...
                return (
                    <div key={i} className="flex gap-3 items-start">
                        ...
                        <div><h4 className="font-bold text-sm text-indigo-100">{rec.title}</h4><p className="text-xs text-slate-400 mt-1">{rec.description}</p></div>
                    </div>
                );
            })}
        </div>
